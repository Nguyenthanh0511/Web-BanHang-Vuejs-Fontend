  ...
  export default {
    ...
    methods:{
         async updateServerData() {
          try {
            // Make a separate request to update the server with the latest data
            await axios.post('http://localhost:3000/updateData', this.$store.state);

            // Log success
            console.log('Data updated on the server:', this.$store.state);
          } catch (err) {
            // Handle errors
            console.error('Error updating data on the server: ', err);
          }
        },
        async signup(e){
            e.preventDefault();
